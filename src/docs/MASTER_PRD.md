...
### 7.1 接続仕様の定義
- **モデル指定**: 世代交代や提供終了に伴う不一致（404エラー）を防ぐため、特定のバージョン番号（1.5等）ではなく、汎用エイリアス `gemini-flash-latest` を原則として使用する。
- **SDK初期化**: APIバージョン（v1/v1beta）のハードコードを避け、`@google/generative-ai` SDKのデフォルト挙動（自動判別）を優先する。

### 7.2 保守ガイドライン（将来の仕様変更への備え）
- **モデル未検出時の対応**: 将来的に再び 404 エラーが発生した場合は、コード修正の前に「現在のAPIキーで利用可能なモデルリスト」をREST API（`curl`等）やSDKで出力し、有効なモデル名と現在のコードの乖離を真っ先に調査すること。
- **フォールバック設計**: 主要モデル（Flash）が応答しない場合、一時的に汎用モデル（Pro等）へ切り替え可能な柔軟な実装を維持すること。
